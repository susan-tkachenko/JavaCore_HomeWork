...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        List<WeatherResponse> fiveDayForecasts;
        try {
            fiveDayForecasts = getFiveDayForecastsJson(locationKey);
        } catch (RuntimeException ex) {
            System.out.println("Не удалось прочитать данные с сервера.");
            throw ex;
        }

        for (WeatherResponse dayForecast : fiveDayForecasts) {
            System.out.printf(
                    "В городе %s на дату %s ожидается %s, %s, температура - %s и %s%n",
                    city,
                    dayForecast.getDate(),
                    dayForecast.getDayTextDescription(),
                    dayForecast.getNightTextDescription(),
                    dayForecast.getMinimumTemperature(),
                    dayForecast.getMaximumTemperature()
            );
        }
    }
    ...
    private static List<WeatherResponse> getFiveDayForecastsJson(String locationKey) {
        ...
        return WeatherResponse.parse(responseJsonReader.readObject());
    }
